...
STATSIG_VERSION = 3
STATSIG_META_B64 = (
    "51uuUc0SPH5sqe5zW5ZbeftUxAhwAnI2U+/Yo1WoX2LguucQ1h19ekHk+fvf26Eg"
)
STATSIG_FINGERPRINT = (
    "19dadc00f5c28f5c28f5c1100f5c28f5c28f5c00"
)
STATIC_STATSIG_ID = base64.b64encode(
...
def _full_version(ua: Optional[str]) -> Optional[str]:
    match = re.search(r"(?:Chrome|Chromium|CriOS|Edg)/(\d+\.\d+\.\d+\.\d+)", ua or "")
...
def _platform_version(ua: str) -> Optional[str]:
    u = ua.lower()
    if "windows nt 10.0" in u:
        return "19.0.0"
    if "windows nt" in u:
        return "15.0.0"
    return None
...
def _client_hints(browser: Optional[str], ua: Optional[str]) -> dict[str, str]:
    ...
    full = _full_version(ua)
    if full:
        hints["Sec-Ch-Ua-Full-Version"] = f'"{full}"'
        hints["Sec-Ch-Ua-Full-Version-List"] = (
            f'"{brand}";v="{full}", "Chromium";v="{full}", "Not)A;Brand";v="24.0.0.0"'
        )
    platform_version = _platform_version(ua or "")
    if platform_version:
...
def build_sso_cookie(
    ...
    *,
    ...
    has_sso = re.search(r"(?:^|;\s*)sso=", eff_cookies)
    has_sso_rw = re.search(r"(?:^|;\s*)sso-rw=", eff_cookies)
    if has_sso or has_sso_rw:
        parts = []
        if not has_sso:
            parts.append(f"sso={tok}")
        if not has_sso_rw:
            parts.append(f"sso-rw={tok}")
        parts.append(eff_cookies)
        return "; ".join(parts)

    return f"{cookie}; {eff_cookies}"
